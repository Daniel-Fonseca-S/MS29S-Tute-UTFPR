...
export const GameUI =
{
  ...
  repaint: function () {
    ...
    for (const card of GameUI.cards.sort(function (a, b) { return a.z - b.z; })) { svgCanvas += '<use id="' + card.getCardName() + '-ID" xlink:href="#' + card.getCardName() + '"/>\n'; }
  ...
  registerCardEvents: function () {
    const f = function (event) {
      if (GameUI.onCardClick !== null) { GameUI.onCardClick(event.data.card); }
    };

    const fIn = function (event) {
      if (GameUI.onCardMouseEnter !== null) { GameUI.onCardMouseEnter(event.data.card); }
    };

    const fOut = function (event) {
      if (GameUI.onCardMouseLeave !== null) { GameUI.onCardMouseLeave(event.data.card); }
    };
