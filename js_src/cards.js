...
export class Card {
  ...
  stepTo(x, y, rotation, moveSpeed, rotSpeed) {
    ...
    let newX = 0; let newY = 0; let newRotation = 0;

    if (d < moveSpeed) {
      newX = x;
      newY = y;

      if (rotSpeed !== 0) { newRotation = this.rotation + rotSpeed; } else { newRotation = rotation; }
    } else {
  ...
  setHighlightColor(color) {
    ...
    if (this.highlightingEnabled && color !== null) { $('#' + cardName + '-RECT').attr('stroke', color); }
  }
